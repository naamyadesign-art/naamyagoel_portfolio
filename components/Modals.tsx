...
const ModalWrapper: React.FC<{ children: React.ReactNode; onClose: () => void; title: string; subtitle?: string; color?: string }> = ({ children, onClose, title, subtitle, color = "bg-white" }) => (
  <div className={`${color} rounded-3xl max-w-5xl w-full text-[#00474F] shadow-2xl relative overflow-hidden h-[85vh] flex flex-col animate-in fade-in zoom-in duration-300 border-2 border-[#00474F]/5`}>
    
    <div className="flex items-center justify-between px-8 py-6 z-50 bg-white/80 backdrop-blur-sm border-b border-[#00474F]/5">
      <div className="flex items-center gap-4">
        <div className="w-8 h-8 bg-[#81D8D0] rounded-full shadow-[0_0_10px_rgba(129,216,208,0.3)]" />
      ...
      <button 
        onClick={onClose} 
        className="w-10 h-10 bg-[#00474F] text-white rounded-full flex items-center justify-center text-xl font-black hover:bg-[#81D8D0] transition-colors cursor-pointer"
      >
        ×
      </button>
    </div>

    <div className="flex-1 overflow-y-auto custom-scrollbar p-8 sm:p-16 relative bg-white">
      <div className="max-w-4xl mx-auto">
        <header className="mb-16">
          <p className="text-[10px] font-black tracking-[0.6em] uppercase text-[#00474F] mb-4">{subtitle}</p>
    ...
    <div className="px-8 py-4 flex justify-between items-center bg-white border-t border-[#00474F]/5">
      <div className="flex gap-2">
        <div className="w-2 h-2 rounded-full bg-[#00474F]/10" />
        <div className="w-2 h-2 rounded-full bg-[#00474F]/10" />
...
export const BioModal: React.FC<ModalProps> = ({ onClose }) => {
  ...
  return (
    <ModalWrapper onClose={onClose} title="About" subtitle="Identity Log" color="bg-white">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        <div className="space-y-12">
          <p className="text-3xl sm:text-5xl leading-tight font-black text-[#00474F]">
            Designing the <span className="text-[#81D8D0] italic font-serif-elegant">unexpected</span> with technical precision.
          </p>
          <p className="text-base text-[#00474F]/60 leading-relaxed font-medium">
          ...
          <div className="space-y-8">
             <div className="border-l-2 border-[#81D8D0] pl-6 py-2">
             ...
             <div className="border-l-2 border-[#00474F] pl-6 py-2">
...
export const ProjectsModal: React.FC<ModalProps & { category: SectionType }> = ({ onClose, category }) => {
  ...
  return (
    <ModalWrapper onClose={onClose} title={category.toString()} subtitle="Creative Works" color="bg-white">
      <div className="space-y-32">
        {filtered.length > 0 ? filtered.map((project, idx) => (
          <div key={project.title} className="group flex flex-col gap-12">
            <div className="relative w-full aspect-[16/10] bg-white rounded-3xl overflow-hidden shadow-xl border border-[#00474F]/5">
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-6">
                 <span className="text-4xl font-serif-elegant italic text-[#81D8D0]">0{idx + 1}.</span>
                 <h3 className="text-4xl sm:text-6xl font-black uppercase">{project.title}</h3>
              </div>
              <p className="text-lg font-bold text-[#00474F] tracking-widest uppercase italic">{project.tagline}</p>
              <p className="text-[#00474F]/60 text-base leading-relaxed font-medium max-w-3xl">{project.description}</p>
              
              <div className="flex flex-wrap gap-4 pt-4">
                {project.tech.map(t => (
                  <span key={t} className="px-6 py-2 bg-white rounded-full border border-[#00474F]/10 text-[10px] font-black uppercase tracking-widest hover:bg-[#00474F] hover:text-white transition-all shadow-sm">
                    {t}
                  </span>
                ))}
              </div>
...
export const GhostModal: React.FC<ModalProps> = ({ onClose }) => {
  ...
  return (
    <ModalWrapper onClose={onClose} title="Ghost" subtitle="Archive AI" color="bg-white">
      <div className="bg-white p-8 rounded-[2rem] border-2 border-[#00474F]/5 shadow-inner min-h-[500px] flex flex-col justify-between">
        <div className="space-y-8 mb-10 overflow-y-auto max-h-[400px] custom-scrollbar pr-4">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] p-6 rounded-2xl ${m.role === 'user' ? 'bg-[#00474F] text-white shadow-lg' : 'bg-gray-100 text-[#00474F] border border-[#00474F]/5'}`}>
          ...
          {loading && <div className="text-xs animate-pulse font-black text-[#81D8D0]">QUERYING_NEURAL_LINKS...</div>}
        </div>
        ...
        <div className="flex gap-4">
          <input 
            ...
            className="flex-1 bg-gray-50 border border-[#00474F]/10 px-8 py-5 text-[#00474F] outline-none focus:border-[#81D8D0] font-bold uppercase tracking-widest text-sm rounded-2xl"
          />
          <button onClick={handleSend} className="bg-[#00474F] text-white px-10 py-5 font-black uppercase text-sm hover:bg-[#81D8D0] transition-all tracking-widest rounded-2xl shadow-lg">
            SEND
          </button>
        </div>
      </div>
